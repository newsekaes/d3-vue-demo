...
data.nodes.map(d => { d.selected = false })
data.links.map(d => { d.selected = false })
const vm = new Vue({
  el: '#app',
  template: '<net :net="net" :width="width" :height="height"/>',
  components: {net},
  data: {
    width,
    height,
...
const simulation = d3.forceSimulation()
  ...
  .force('charge', d3.forceCollide(30).strength(0.7))
simulation
  .nodes(data.nodes)
  // .on('tick', ticked)
  .on('end', () => { console.timeEnd('tick') })
vm.$data.net = data
